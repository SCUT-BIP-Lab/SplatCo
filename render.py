...
def render_sets(dataset : ModelParams, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool, decompress:bool):
    with torch.no_grad():
        ...
        scene = Scene(dataset, gaussians, load_iteration=iteration, shuffle=False, skip_test=args.skip_test, skip_train=args.skip_train, decompress=decompress)
        gaussians.eval()
        #gaussians.magic_k = True
        gaussians.magic_k = False
        gaussians.enable_net = True
        gaussians.feat_planes.Q0 = 0
        gaussians.feat_planes.activate_plane_level()
        gaussians.feat_planes.activate_plane_level()
        ...
        num_gaussians = scene.gaussians.get_anchor.shape[0]
...
if __name__ == "__main__":
    ...
    render_sets(model.extract(args), args.iteration, pipeline.extract(args), args.skip_train, args.skip_test, args.decompressed)